...
如果用户已经给了房间号和订阅模式，但还没写备注，插件会继续追问：
...
如果用户在群里以普通成员身份发起订阅流程，插件会在一开始直接拒绝，不再继续询问模式或备注：

- `只有群管理员才能在群里创建订阅。普通成员如需订阅，请先添加机器人好友后在私聊中发起。`
...
6. 如果订阅模式已经明确、但还没提供备注，插件会继续追问备注，支持用户回复备注名或 `跳过`
7. 如果带了 `备注/别名/昵称`，插件会一并记录，并在提醒时优先显示
8. 进入追问阶段后，用户继续直接回复即可，不必再次 `@机器人`
9. 创建群订阅前会校验当前发言者是否为群管理员
10. 创建私聊订阅前会要求用户在私聊中发起，以确保机器人已加好友
11. 订阅成功时会直接返回确认文本，并尽量附带当前直播房封面
12. 如果订阅时直播间已经在播，确认消息会明确提示“当前正在直播”
13. 订阅成功后，后台定时任务按 `check_interval_seconds` 轮询直播间状态
14. 若直播状态从非开播变为开播，则发送开播提醒
15. 若直播状态从开播变为非开播，则发送下播提醒
